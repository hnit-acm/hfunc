...
type StructFormatter func() (hbasic.StringFormatFunc, CacheFunc, LayerSplitFunc)

type LayerSplitFunc func() string

type CacheFunc func() (hbasic.GetFunc, hbasic.SetFunc)

// DefaultGetFieldsArray 默认方法
var DefaultGetFieldsArray, DefaultGetFieldsString = NewGetFields(
	func() (hbasic.StringFormatFunc, CacheFunc, LayerSplitFunc) {
		LayerSplitFunc := LayerSplitFunc(func() string {
			return ","
		})

		CacheFunc := CacheFunc(func() (hbasic.GetFunc, hbasic.SetFunc) {
			get, set, _ := hbasic.NewHashMapFunc(1024)
			return get, set
		})

		return hbasic.SnakeCasedStringFormat,
			CacheFunc,
			LayerSplitFunc
	},
)

type GetFieldsArrayFunc func(p interface{}, cacheNo string) hbasic.ArrayString
...
func NewGetFields(structFormatter StructFormatter) (GetFieldsArrayFunc, GetFieldsStringFunc) {
	...
	get, set := cache()
	GetFieldsArrayFunc := GetFieldsArrayFunc(func(p interface{}, cacheNo string) hbasic.ArrayString {
		t := reflect.ValueOf(p).Elem()
		path := cacheNo + t.Type().PkgPath() + "." + t.Type().String()
		//key := crc32.ChecksumIEEE([]byte(path))
		if val, ok := get(path); ok {
			return val.(hbasic.ArrayString)
		}
		all := make(hbasic.ArrayString, 0)
	...
	GetFieldsStringFunc := GetFieldsStringFunc(func(p interface{}, splitChar string, cacheNo string) string {
		...
		//all := ArrayStringToStringBuilder(harray, splitChar)
		// store cache
		set(pathStr, all)
		return all
	})
...
func layerHandle(t reflect.Value, all []string, prefix string, format hbasic.StringFormatFunc, split LayerSplitFunc) []string {
	num := t.NumField()
	for i := 0; i < num; i++ {
		tf := t.Field(i)
		tt := t.Type().Field(i)
...
var DefaultStructToMap = NewStructToMap(
	func() (hbasic.StringFormatFunc, CacheFunc, LayerSplitFunc) {
		return hbasic.SnakeCasedStringFormat, nil, nil
	},
)
